...
public class Signpost : MonoBehaviour
{
    ...
    [SerializeField] GameObject signpostMenu;
    ...
    void Update()
    {
        if (Time.timeScale != 0)
        {
            ...
            if (distance < activationDistance)
            {
                ...
                if (Input.GetKeyDown(KeyCode.E) || Input.GetKeyDown(KeyCode.F) && buffer <= 0) // if press E or F
                {
                    buffer = 2;
                    signpostMenu.GetComponent<SignpostMenu>().Activate();
                }
                if (buffer > 0)
                    buffer--;
            }
